...
- [ ] 034. **[CORE_GDD_02]** `IPO_Service.cs`: 상폐 공석 발생 시 대기 종목 풀에서 신규 상장 트리거
- [ ] 035. **[CORE_GDD_03]** `WalletManager.cs`: 가용 현금, 누적 이자, 미지급 배당금의 실시간 정산부
- [ ] 036. **[CORE_GDD_03]** `NetWorthCore.cs`: 포트폴리오 평가액 + 부동산 + 현물 자산 합산 연산 엔진
- [ ] 037. **[CORE_GDD_03]** `LevelEngine.cs`: 누적 거래액 기반 레벨업 및 스탯 포인트 지급 루틴
- [ ] 038. **[CORE_GDD_03]** `StatCore.cs`: 분석력, 운, 협상력 등 5대 능력치 갱신 및 데이터 보존부
...
- [ ] 042. **[CORE_GDD_01]** `GlobalPostPro`: 씬별 포스트 프로세싱(Vignette, Bloom, CRT) 프로필 설정
- [ ] 043. **[CORE_GDD_01]** `CursorManager`: 상황별 커서 아이템(돋보기, 손바닥) 변경 및 애니메이션
...
- [ ] 047. **[CORE_GDD_05]** `UI_SafeArea`: 노치 디자인 및 다양한 종횡비 대응 캔버스 조절 스크립트
...
- [ ] 051. **[CORE_GDD_02]** `PriceNoise.cs`: 주간 평탄화 사이사이의 미세한 '노이즈' 가격 변동 연산부
- [ ] 052. **[CORE_GDD_02]** `MarketOpeningFX`: 개장 시 증권사 UI가 반짝이며 켜지는 시계 연출
- [ ] 053. **[CORE_GDD_06]** `DataIntegrity.cs`: 매 프레임 세이브 데이터 변조를 감시하는 무결성 엔진
...
- [ ] 059. **[CORE_GDD_04]** `LoanEvaluator`: 총자산/평판 대비 대출 가능 한도(최대 10,000G 초기값) 연산
- [ ] 060. **[CORE_GDD_04]** `AutoRepayment.cs`: 배당금 발생 시 부채 원금을 우선 차감하는 자동 이체부
- [ ] 061. **[CORE_GDD_04]** `ManualRepayment.cs`: 유저 입력 기반의 부분 상환 및 상환 후 이윤 재계산
- [ ] 062. **[CORE_GDD_04]** `MarginShortLogic`: 공매도 시 주문가 150% 현금 동결 및 담보물 설정 로직
- [ ] 063. **[CORE_GDD_04]** `MarginCallEngine`: 유지비율 90% 도달 시 경고, 100% 도달 시 강제 청산 루틴
- [ ] 064. **[CORE_GDD_04]** `SeizureManager`: 자산 0 이하 시 주식->가구 순의 순차적 압류 실행 엔진
...
- [ ] 067. **[CORE_GDD_05]** `MainHUD_Master`: 상단 바, 사이드 메뉴를 포함한 핵심 HUD 레이아웃 셋업
...
- [ ] 071. **[CORE_GDD_05]** `TickerColorFX`: 상승(Cyan)/하락(Peach) 텍스트 색상 및 글로우 셰이더 적용
- [ ] 072. **[CORE_GDD_05]** `AreaChart_Core`: Mesh API 기반의 실시간 주가 선 및 면 선형 렌더러
...
- [ ] 075. **[CORE_GDD_05]** `GlobalLighting_Cycle`: 게임 시간대에 따른 실시간 환경 광원(Day/Night) 변화
- [ ] 076. **[CORE_GDD_05]** `RainFX_Controller`: 날씨 시스템과 연동된 창밖 빗줄기 입자 및 바닥 물튀김
- [ ] 077. **[CORE_GDD_05]** `WindowRainShader`: 유리창에 흘러내리는 물방울의 굴절 셰이더 디테일 작업
- [ ] 078. **[CORE_GDD_05]** `GlitchCore`: 뉴스 발생 시 UI가 일시적으로 깨지는 도트 글리치 컴포넌트
- [ ] 079. **[CORE_GDD_05]** `Anna_IdleState`: 홈 오피스 내 안나의 대기 위치 설정 및 기본적인 숨쉬기 애니
- [ ] 080. **[CORE_GDD_08]** `CharacterSelectUI`: 초기 외형(머리, 안경, 의상) 선택을 위한 갤러리 뷰
...
- [ ] 083. **[CORE_GDD_06]** `AssetDissolveFX`: 가구 압류 시 입자가 흩어지며 사라지는 디졸브(Dissolve) 효과
...
- [ ] 087. **[CORE_GDD_05]** `SceneNavigationUI`: 지하철 노선도 테마의 맵 선택 창 및 장소별 썸네일 노출
- [ ] 088. **[CORE_GDD_05]** `TooltipManager`: 모든 아이콘 마우스 오버 시 정보를 뿌려주는 전역 툴팁 시스템
- [ ] 089. **[CORE_GDD_03]** `LevelUpVisual`: 레벨 상승 시 화면 중앙에 나타나는 금빛 엠블럼과 보상 연출
...
- [ ] 092. **[CORE_GDD_05]** `ResolutionScaler`: 720p ~ 4K까지의 UI 스프라이트 해상도 최적화 대응
- [ ] 093. **[CORE_GDD_02]** `StatusLED.cs`: 장 열림(Green)/닫힘(Red)을 표시하는 책상 위 기계 장치 연출
...
- [ ] 105. **[CORE_GDD_02]** `CandleStick_Renderer`: 봉 차트 보기 모드 전환을 위한 시가/고가/저가/종가 계산부
- [ ] 106. **[CORE_GDD_05]** `UI_BlurEffect`: 창이 뜰 때 배경을 은은하게 흐리게 만드는 가우시안 블러 셰이더
- [ ] 107. **[CORE_GDD_04]** `AutomaticSeizureMail`: 압류 6시간 전 자동으로 발송되는 최후의 결제 독촉 알림 로직
- [ ] 108. **[CORE_GDD_05]** `Ticker_NewsSymbol`: 실시간으로 터지는 호재/악재 뉴스 아이콘을 티커에 삽입
- [ ] 109. **[CORE_GDD_06]** `SaveSafetyCheck`: 저장 도중 전원 차단 시 직전 세이브 데이터를 보호하는 백업본 생성
...
- [ ] 176. **[MOD_GDD_03]** `ItemMasterTable`: CSV 기반 가구/의상 200종 스탯 대량 로드 시스템
...
- [ ] 183. **[MOD_GDD_04]** `RumorGenerator`: 알바 성공 시 48종 시나리오 중 확률적 찌라시 획득
- [ ] 184. **[MOD_GDD_04]** `RumorInventory`: 획득한 찌라시 전용 수집함 및 열람 인터페이스
- [ ] 185. **[MOD_GDD_04]** `BurnTimerLogic`: 열람 후 60분 뒤 인벤토리에서 자동 삭제되는 타이머
- [ ] 186. **[MOD_GDD_04]** `InsightMaskingEngine`: 분석 레벨 1~3단계별 단어 은폐 및 치환 알고리즘
- [ ] 187. **[MOD_GDD_04]** `RumorExpirationFX`: 만료 임박(5분 전) 찌라시 아이콘의 붉은 깜빡임 연출
- [ ] 188. **[MOD_GDD_04]** `DecoderItemAction`: '찌라시 판독기' 사용 시 마스킹 해제 실시간 연출
- [ ] 189. **[MOD_GDD_05]** `WardrobeUI`: 현재 착용 중인 의상 세트 효과 및 총 버프 계산창
- [ ] 190. **[MOD_GDD_05]** `PawnLoanProcess`: 바터에게 물건 맡기고 구입가 60% 대출받는 프로세스
- [ ] 191. **[MOD_GDD_05]** `DefaultPawnInterest`: 담보 대출 전용 매주 5% 복리 이자 합산 정산 루틴
- [ ] 192. **[MOD_GDD_05]** `PawnItemStorage`: 담보로 맡긴 물건들을 따로 보관하는 가상 저장소
- [ ] 193. **[MOD_GDD_05]** `RedemptionProcess`: 원금+이자 상환 후 담보물 인벤토리 재지급 로직
- [ ] 194. **[MOD_GDD_05]** `BarberShopUI`: 헤어 스타일 및 컬러 변경 프리뷰 및 자본 차감 로직
...
- [ ] 198. **[MOD_GDD_03]** `ItemRarityFX`: 레전더리 등급 아이템 획득 시 화면 전체 아우라 연출
- [ ] 199. **[MOD_GDD_04]** `RumorSourceTag`: 찌라시 출처(다크넷, 브로커, 우연) 표기 로직
- [ ] 200. **[MOD_GDD_04]** `ReliabilitySystem`: 찌라시 텍스트 중 '거짓'이 섞일 확률 관리 엔진
...
- [ ] 204. **[MOD_GDD_04]** `InformationPurge`: 만료 시 정보가 타 들어가며 사라지는 셰이더 효과
...
- [ ] 209. **[MOD_GDD_04]** `CriticalRumorAlert`: 시장 전체에 영향을 주는 빨간색 '치명적 찌라시' 연출
- [ ] 210. **[MOD_GDD_03]** `InventoryExpansion`: 서랍장 칸 늘리기 아이템 사용 시 그리드 확장 연출
...
- [ ] 220. **[MOD_GDD_05]** `PawnShopBargain`: 바터와의 [협상력] 대결을 통해 담보 대출 이율을 깎는 미니 토크
...
- [ ] 236. **[MOD_GDD_09]** `VirtualFriendNet`: 가상의 친구 유저 10명의 프로필 및 가짜 투자 일지 자동 생성기
...
- [ ] 294. **[CORE_GDD_05]** `DrawingCeremonyUI`: 21:00 광원 차단 후 나타나는 화려한 추첨 머신 윈도우
...
- [ ] 315. **[Balance]** 1,000회 이상의 오토 플레이 시뮬레이션을 통한 후반부 경제 정체 구간 해소
- [ ] 316. **[Optim]** 텍스처 아틀라스 압축 최적화 및 비사용 모델링 에셋 스트리밍 시스템 점검
- [ ] 317. **[Optim]** CPU 프로필링을 통한 실시간 주가 연산 시 메모리 할당(Alloc) 0MB 지향 최적화
- [ ] 318. **[Optim]** 모바일 발열 방지를 위한 최대 프레임(30/60) 선택 및 자동 가변 조절 트리거
...
- [ ] 322. **[Test]** 저사양 안드로이드 기기에서의 UI 렌더링 지연 시간(Input Lag) 측정 및 보정
- [ ] 323. **[Final]** 모든 태스크 `- [x]` 완료 여부 및 각 GDD 문서와의 상호 정합성 전수 대조
- [ ] 324. **[Final]** 릴리즈 패키징용 안드로이드/iOS/Windows/Mac 크로스 플랫폼 빌드 수행
- [ ] 325. **[Final]** 개발 기획서와 최종 빌드 간의 사양 차이점(Spec Diff) 문서 최종 정리
...
- [ ] 334. **[Social]** 커뮤니티 배포용 홍보용 고화질 스크린샷 10종 및 트레일러 캡처
...
- [ ] 347. **[Phase7]** 코인(Crypto) 테마의 신규 섹터 및 변동성 미분 로직 추가 설계
